...
class ComputerBreaker
  ...
  def check_possible_code?(clues, guess_code, possible_code)
    ...
    duplicate_possible_code = possible_code.to_s.split('').map(&:to_i)

    duplicate_possible_code.each_with_index do |digit, index|
      next unless digit == duplicate_guess_code[index]
    ...
    duplicate_possible_code.each_with_index do |digit, index|
      ...
      next if matched_guess_index.nil?
    ...
    clues == [gray_peg_count, red_peg_count]
  end
  ...
  def break_code(turn)
    if turn == 1
      ...
      @possible_codes[0].to_s.split('').map(&:to_i)
    end
  end
end
...
class Game
  ...
  def introduction
    ...
    gray_peg
    print "This clue means you have 1 correct number at the correct location.\n"
    red_peg
    print "This clue means you have 1 correct number but at the wrong location.\n"
  ...
  def maker_gameplay
    ...
    puts 'Congratulations You Win, the computer cannot break your code within 12 turns.' if win == true
  end
  ...
  def give_clue(maker_code, breaker_code)
    ...
    duplicate_breaker_code.each_with_index do |digit, index|
      next unless digit == duplicate_maker_code[index]
    ...
    duplicate_breaker_code.each_with_index do |digit, index|
      ...
      next if matched_maker_index.nil?
